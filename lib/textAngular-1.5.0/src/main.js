...
textAngular.run([function(){
	...
	if (typeof define === 'function' && define.amd) {
		...
		if(!window.rangy){
			throw("rangy-core.scripts and rangy-selectionsaverestore.scripts are required for textAngular to work correctly, rangy-core is not yet loaded.");
		}else{
			window.rangy.init();
			if(!window.rangy.saveSelection){
				throw("rangy-selectionsaverestore.scripts is required for textAngular to work correctly.");
			}
		}
	}
}]);
				...
				scope.showResizeOverlay = function(_el){
					...
					_resizeMouseDown = function(event){
						...
						oneEvent(_body, 'mouseup', function(event){
							...
							// at this point, we need to force the model to update! since the styles has changed!
